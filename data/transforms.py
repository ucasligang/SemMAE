...
class RandomHorizontalFlip(torch.nn.Module):
    ...
    def forward(self, img):
        ...
        if torch.rand(1) < self.p:
            ...
            sorted_index = 28*(sorted_index//28)+(27-sorted_index%28)
            x[:, 14:15] = sorted_index
            patches = x[:, 15:]  # [196, 192]
            flipped_patches = patches.reshape(196, 8, 8, 3)[:, :, ::-1, :].reshape(196, -1)
            x[:, 15:] = flipped_patches
            return x
        return img
